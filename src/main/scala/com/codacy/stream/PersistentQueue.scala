...
/**
  ...
  */
class PersistentQueue[T](config: QueueConfig, onCommitCallback: Int => Unit = _ => {}, indexName: String = "tailer.idx")(
  ...
  def this(config: Config, indexName: String)(implicit serializer: QueueSerializer[T]) =
    this(QueueConfig.from(config), indexName = indexName)

  def this(persistDir: File)(implicit serializer: QueueSerializer[T]) = this(QueueConfig(persistDir))

  def this(persistDir: File, indexName: String)(implicit serializer: QueueSerializer[T]) =
    this(QueueConfig(persistDir), indexName = indexName)
