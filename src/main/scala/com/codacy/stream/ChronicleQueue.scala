...
class ChronicleQueue[T](queue: PersistentQueue[T], onPushCallback: () => Unit)(
  ...
  def this(queue: PersistentQueue[T])(implicit serializer: QueueSerializer[T], system: ActorSystem) =
    this(queue, () => {})

  def this(config: Config)(implicit serializer: QueueSerializer[T], system: ActorSystem) =
    this(new PersistentQueue[T](config))

  def this(config: Config, indexName: String)(implicit serializer: QueueSerializer[T], system: ActorSystem) =
  ...
  def this(persistDir: File, indexName: String)(implicit serializer: QueueSerializer[T], system: ActorSystem) =
...
object ChronicleQueue {
  ...
  def apply[T](config: Config, indexName: String)(implicit serializer: QueueSerializer[T], system: ActorSystem) =
    new ChronicleQueue[T](config, indexName = indexName)

  def apply[T](persistDir: File, indexName: String)(implicit serializer: QueueSerializer[T], system: ActorSystem) =
    new ChronicleQueue[T](persistDir, indexName = indexName)

  def source[T](persistDir: File)(implicit serializer: QueueSerializer[T], system: ActorSystem) =
    Source.fromGraph(new ChronicleQueueSource[T](new PersistentQueue[T](persistDir)))

  def source[T](config: Config)(implicit serializer: QueueSerializer[T], system: ActorSystem) =
    Source.fromGraph(new ChronicleQueueSource[T](new PersistentQueue[T](config)))

  def source[T](persistDir: File, indexName: String)(implicit serializer: QueueSerializer[T], system: ActorSystem) =
    Source.fromGraph(new ChronicleQueueSource[T](new PersistentQueue[T](persistDir, indexName = indexName)))

  def source[T](config: Config, indexName: String)(implicit serializer: QueueSerializer[T], system: ActorSystem) =
    Source.fromGraph(new ChronicleQueueSource[T](new PersistentQueue[T](config, indexName = indexName)))

  def sink[T](persistDir: File)(implicit serializer: QueueSerializer[T], system: ActorSystem) =
    Sink.fromGraph(new ChronicleQueueSink[T](new PersistentQueue[T](persistDir, indexName = null)))

  def sink[T](config: Config)(implicit serializer: QueueSerializer[T], system: ActorSystem) =
